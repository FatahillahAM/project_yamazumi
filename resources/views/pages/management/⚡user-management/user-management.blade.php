<div class="w-full bg-white dark:bg-neutral-900 shadow-xl rounded-xl overflow-hidden 
    ...
    <div class="p-6 space-y-4">
        <div class="flex justify-between items-center py-4">
            ...
            <div class="flex items-center gap-4">
                ...
                {{-- BUTTON --}}
                @can('create_user')
                <flux:button icon="plus" wire:click="createUser" class="!bg-blue-600 hover:!bg-blue-700 !text-white text-sm px-3 py-2 rounded-md shadow-sm transition-all duration-200">
                    Tambah user
                </flux:button>
                @endcan
        ...
        <flux:table container:class="max-h-[70vh]" :paginate="$this->users" class="min-w-full">
            ...
            <flux:table.rows>
                ...
                <flux:table.row wire:key="user-{{ $user->id }}" class="hover:bg-gray-50 dark:hover:bg-neutral-800 transition odd:bg-white even:bg-gray-50 dark:odd:bg-gray-900/50 dark:even:bg-gray-950">
                    ...
                    <flux:table.cell align="center">{{ $user->roles->first()?->name ?? '-' }}</flux:table.cell>
                    <flux:table.cell align="center">
                        <div class="flex justify-center items-center gap-2">
                            @can('update_user')
                            <flux:modal.trigger name="edit-user-{{ $user->id }}">
                                <flux:button icon="pencil-square" icon:variant="solid" tooltip="Edit user" tooltip:position="top" wire:click="edit({{ $user->id }})" size="sm" variant="primary" color="blue" />
                            </flux:modal.trigger>
                            @endcan
                            @can('delete_user')
                            <flux:button icon="trash" icon:variant="solid" tooltip="Hapus user" tooltip:position="top" wire:click="confirmDelete({{ $user->id }})" size="sm" variant="danger" />
                            @endcan
                        </div>
                    </flux:table.cell>
                </flux:table.row>
    ...
    <flux:modal wire:model="showUserModal" flyout variant="floating" class="md:w-2xl">
        <form wire:submit.prevent="save" class="space-y-6">
            ...
            {{-- EMAIL --}}
            <flux:input type="email" label="Email Pengguna" placeholder="Masukkan email pengguna" wire:model.defer="email" />
            @error('email')
            <flux:text size="sm" class="text-rose-500 -mt-2">
                {{ $message }}
            </flux:text>
